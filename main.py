...
with col2:
    if predict_button and model is not None and preprocessor is not None:
        ...
        # Ensure column order is correct for preprocessing
    ...
    if 'prediction' in st.session_state:
        ...
        colors = ['#f44336', '#4CAF50'] # Red for disease, Green for healthy
        ...
        st.pyplot(fig)
        plt.close(fig) # Always close figures to free up memory
        
        # --- SHAP Explanation ---
        st.subheader("🔬 Feature Contribution Analysis (SHAP)")
        try:
            explainer = shap.TreeExplainer(model) # TreeExplainer for tree-based models like XGBoost
            X_transformed_df = pd.DataFrame(st.session_state['transformed_data'], columns=user_input.columns) # Use original column names
            shap_values = explainer.shap_values(X_transformed_df)
            
            # For binary classification, shap_values can be a list of arrays (one for each class).
            # We usually plot the SHAP values for the "positive" class (class 1 - disease).
            if isinstance(shap_values, list):
                shap_values_to_plot = shap_values[1] # Assuming class 1 is positive/disease
            else:
            ...
            st.pyplot(fig_shap)
            plt.close(fig_shap) # Always close figures

            st.info("""
            **How to read this chart:** This waterfall plot shows how each feature contributed to pushing the model's prediction from a baseline value to the final output (likelihood of disease).
            - **Red bars** represent features that increase the likelihood of heart disease.
            - **Blue bars** represent features that decrease the likelihood.
            - The length of the bar indicates the magnitude of the feature's impact.
            """)
        except Exception as e:
            st.warning(f"Could not generate SHAP plot: {str(e)}. This often happens if the model is not compatible with SHAP Explainer or due to data issues.")
            st.exception(e) # Display full error for debugging
